...
<shader language="GLSL">
    <fragment><![CDATA[
        ...
        void main()
        {
                ...
                vec4 wid = 2 + 2 * pow(col, 4.0);
                vec4 weights = uv_ratio.y/0.3;
                weights = 0.51*exp(-pow(weights*sqrt(2/wid),wid))/0.3/(0.6+0.2*wid);
                wid = 2 + 2 * pow(col2,4.0);
                vec4 weights2 = (1.0-uv_ratio.y)/0.3;
                ...
                gl_FragColor = pow(mcol*(col * weights + col2 * weights2), 1.0/2.2);
        }
    ]]></fragment>
</shader>
